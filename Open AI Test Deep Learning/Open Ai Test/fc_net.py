...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    y2, cache2 = affine_relu_forward(y1, self.params['W2'], self.params['b2'])
    scores = y2

    loss1, dscores = softmax_loss(scores, y)
    dy2, dw2, db2 = affine_relu_backward(dscores, cache2)
    dy1, dw1, db1 = affine_relu_backward(dy2, cache1)
    dw2 += self.reg * self.params['W2']   # Do not forget regularization
    dw1 += self.reg * self.params['W1']

    loss2 = loss1 + (self.reg/(2.0))*(np.sum(self.params['W1']*self.params['W1']) + np.sum(self.params['W2']*self.params['W2']))
    ...
    loss, grads = loss2, {'W1': dw1, 'W2': dw2, 'b1': db1, 'b2': db2}
    ############################################################################
...
class FullyConnectedNet(object):
  ...
  def __init__(self,
    ...
    self.reg = reg
    self.num_layers = 1 + len(self.hidden_dims)
    self.dtype = dtype
    self.params = {}
    ...
    for i in range(self.num_layers):
      ...
      bias = 'b' + str(i+1)
      if i+1 == 1:
        w = np.random.normal(0, weight_scale, (self.input_dim,
                                               self.hidden_dims[i]))
        b = np.zeros((hidden_dims[i],))
      elif i+1 == self.num_layers:
        w = np.random.normal(0, weight_scale, (self.hidden_dims[i-1],
                                               self.num_classes))
        ...
        w = np.random.normal(0, weight_scale, (self.hidden_dims[i-1],
                                               self.hidden_dims[i]))
        b = np.zeros((hidden_dims[i],))
      self.params.update({W: w})
      self.params.update({bias: b})

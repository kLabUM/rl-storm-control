...
env = wrappers.Monitor(env, "/Users/abhiram/Desktop/keras/")
...
time = 0
steps = 50000
epsilon = np.linspace(0.0, 0.0, 120000+10)
rewards_episodes = []
network_loss = []
...
while time < steps:
    ...
    train = True if time > 100000 else False
    loss_check = 0
    episode_time = 0
    while not(done) and episode_time < 1000:
        episode_time += 1
        time += 1
...
env.close()
gym.upload("/Users/abhiram/Desktop/keras/", api_key='sk_tHy7ubR2T7mu1KmejCddyA')

...
for i in nodes_controlled.keys():
    model = target = build_network(len(states_controlled[i]),
                                   len(action_space),
                                   2, 50, 'relu', 0.0)
    model.load_weights("controllers/" + i + load_model)
...
for i11 in rain_duration:
    for j11 in return_period:
        ...
        np.save('response_controlled'+i11+"_"+j11+'.npy', controlled_ponds)
        np.save('response_uncontrolled'+i11+"_"+j11+'.npy', uncontrolled_ponds)
        np.save('outflow_last_'+i11+"_"+j11+".npy", outflow_track)
        np.save('actions_'+i11+"_"+j11+".npy", actions_track)

...
time = 0
steps = 500000
episode_numbers = 10000
epsilon = np.linspace(0.3, 0.001, episode_numbers+10)
#  Book Keeping
rewards_episodes = []
network_loss = []
height_pond_mean = []
...
while time < steps:
    ...
    while episode_time < 500 and not(done):
        ...
        action = epsi_greedy(action_space, q_values,
                             epsilon[min(episode_numbers, episode_count)])
...
model.save_weights('single_pond_new_all_in.h5')
plt.figure(1)

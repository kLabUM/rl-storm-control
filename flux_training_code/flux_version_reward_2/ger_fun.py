...
def reward_function(height_pond, outflow_pond, gate_change_rate):
    reward_gate_change = -1.0 if gate_change_rate > 0.4 else 0.0
    if height_pond <= 2.0:
        if outflow_pond <= 0.10:
            return 1.0 - 0.25 * height_pond + 1.0 + reward_gate_change
        else:
            return -10.0*outflow_pond + 2.0 - 0.25 * height_pond + 1.0 + reward_gate_change
    else:
        if outflow_pond <= 0.10:
            return 1.0 - 1.50 * height_pond + 1.0 + reward_gate_change
        else:
            return -10.0*outflow_pond + 2.0 - 1.50*height_pond + 1.0 + reward_gate_change

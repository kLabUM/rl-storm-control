...
class deep_q_agent:
    def __init__(self,
        ...
        self.tatget_model = target_model
        self.replay = replay_memory
        self.policy = policy
        self.batch = batch_size
        self.train = train
        self.target_update = target_update
        self.training_batch = {'states': np.zeros((self.batch, self.states)),
                               'states_new': np.zeros((self.batch,
                                                       self.states)),
                               'actions': np.zeros((self.batch, 1)),
                               'rewards': np.zeros((self.batch, 1)),
                               'terminal': np.zeros((self.batch, 1))}

    def _random_sample(self):
        indx = randombatch(self.batch, self.replay)
    ...
    def train(self, timestep):
        _random_sample()
        if timestep % self.target_update:

...
epi_start = 0.3#float(sys.argv[1])
epi_end = 0.3#float(sys.argv[2])
load_model_name = "i"#sys.argv[3]
save_model_name = "test"#sys.argv[4]
...
def implement_action(gate_settings, nodes, NODES_LIS):
    ...
    return gate_settings[nodes.index(i)]
...
# controlled ponds
con_ponds = ['93-50077','93-90358', '93-90357']

# Gate Positions start with open
gates = gate_positions(con_ponds, 0.1)
...
while episode_tracker < episode_count:
    ...
    while episode_timer < time_sim:
        ...
        acts.append(implement_action(gates.current_gate, con_ponds, NODES_LIS))
        acts_1.append(action)
